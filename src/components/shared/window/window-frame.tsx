  ...
  return (
    ...
    >
      <header className="flex items-center justify-between border-b border-white/10 bg-gradient-to-b from-background/80 to-surface/90 px-3.5 py-1.5 md:px-4 md:py-2">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <p className="min-w-0 truncate text-xs font-medium text-foreground/92 md:text-sm">
            <span>{title}</span>
            {subtitle ? (
              <span className="ml-2 font-mono text-[10px] font-normal text-foreground/55 md:text-[11px]">
                {subtitle}
              </span>
            ) : null}
          </p>
          ...
          <div className="flex shrink-0 items-center gap-1.5 md:gap-2">
            ...
            {help ? (
              <details className="group relative">
                ...
                <div
                  className={cn(
                    "absolute right-0 top-8 z-20 w-72 p-3 backdrop-blur",
                    panelSurfaceElevatedClass
                  )}
                >
                  ...
                  {help.cta ? (
                    <Link
                      href={help.cta.href}
                      className="mt-3 inline-flex text-xs font-semibold text-brand hover:text-brand/80"
                    >
                      {help.cta.label}
                    </Link>
                  ) : null}
      ...
      <div
        className={cn(
          "min-w-0 p-5 md:p-6",
          isFixed ? "md:min-h-0 md:flex-1 md:overflow-y-auto" : undefined,
          bodyClassName
        )}
      >
        {children}
...
function statusBadgeClass(status: WindowFrameStatus): string {
  const base =
    "inline-flex rounded-full border px-1.5 py-0 text-[9px] font-mono uppercase tracking-wide md:text-[10px]";
