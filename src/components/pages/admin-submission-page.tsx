...
import {
  ...
  updateContactSubmissionReviewStatus,
} from "@/lib/api/admin";
...
export function AdminSubmissionPage({
  ...
  const [submission, setSubmission] = useState<ContactSubmissionDetail | null>(
    null
  );
  ...
  async function handleStatusSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await updateContactSubmissionReviewStatus(
        accessToken,
        submission.id,
        {
      ...
      setErrorMessage(
        getApiErrorMessage(error, "The review status could not be updated.")
      );
    } finally {
      setIsUpdating(false);
    }
  ...
  return (
    ...
    >
      ...
      {errorMessage ? (
        <div className="rounded-xl border border-border/80 bg-surface/95 p-4 text-sm font-medium text-[#f0c674] shadow-card">
          {errorMessage}
        </div>
      ) : null}

      {isLoading ? (
        <div className="rounded-xl border border-border/80 bg-surface/95 p-6 text-sm text-foreground/70 shadow-card">
          Loading submission...
        </div>
      ) : null}
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,2fr)_360px]">
          <section className="rounded-xl border border-border/80 bg-surface/95 p-5 shadow-card">
            ...
            <div className="mt-5 rounded-xl border border-border/70 bg-background/20 p-4">
              <p className="whitespace-pre-wrap text-sm leading-6 text-foreground/86">
                {submission.message}
              </p>
            </div>
          </section>
            ...
            <section className="rounded-xl border border-border/80 bg-surface/95 p-5 shadow-card">
              ...
              <form className="mt-4 space-y-3" onSubmit={handleStatusSubmit}>
                ...
                <div aria-live="polite" className="min-h-5 text-sm">
                  {statusMessage ? (
                    <p className="font-medium text-success">{statusMessage}</p>
                  ) : null}
                </div>
                ...
                <Button
                  ...
                  className="w-full"
                  disabled={
                    isUpdating || selectedStatus === submission.reviewStatus
                  }
                >
            ...
            <section className="rounded-xl border border-border/80 bg-surface/95 p-5 shadow-card">
              ...
              <dl className="mt-4 space-y-3 text-sm">
                ...
                <div>
                  ...
                  <dd className="mt-1 text-foreground">
                    {formatDateTime(submission.lastDeliveryAttemptAt)}
                  </dd>
                </div>
                <div>
                  <dt className="text-foreground/60">Delivered at</dt>
                  <dd className="mt-1 text-foreground">
                    {formatDateTime(submission.deliveredAt)}
                  </dd>
                </div>
                <div>
                  <dt className="text-foreground/60">Last delivery error</dt>
                  <dd className="mt-1 whitespace-pre-wrap text-foreground">
                    {submission.lastDeliveryError || "—"}
                  </dd>
                </div>
              </dl>
            </section>
