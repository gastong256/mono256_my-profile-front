...
const socialItems: SocialItem[] = [
  ...
  ...siteContent.social
    .filter((item): item is { label: "GitHub" | "LinkedIn"; href: string } => item.label === "GitHub" || item.label === "LinkedIn")
    ...
    <footer className="shrink-0 border-t border-interactive-border/70 bg-background/45">
      <PageContainer className="py-1">
        <div className="flex items-center justify-between gap-3 text-[10px] text-foreground/55 md:text-[11px]">
          <p className="whitespace-nowrap font-mono lg:hidden">© 2026 {siteContent.domain}</p>
          <p className="hidden whitespace-nowrap font-mono lg:block">© 2026 {siteContent.domain} · Release {releaseLabel} by Mono256</p>
