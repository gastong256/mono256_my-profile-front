...
type WindowFrameStatus = "read-only" | "interactive" | "external-links";
type WindowFrameSize = "auto" | "fixed";
          ...
          <div className="flex shrink-0 items-center gap-1.5 md:gap-2">
            ...
            {help ? (
              <details className="group relative">
                <summary
                  className="inline-flex h-5 w-5 cursor-pointer list-none items-center justify-center rounded-md bg-foreground/10 p-0 text-foreground/80 transition-colors hover:bg-foreground/18 hover:text-foreground"
                ...
                <div className="absolute right-0 top-8 z-20 w-72 rounded-lg border border-border/80 bg-surface/95 p-3 shadow-card backdrop-blur">
...
function statusBadgeClass(status: WindowFrameStatus): string {
  ...
  if (status === "interactive") {
    return `${base} border-[#4fa770]/40 bg-[#4fa770]/15 text-[#8ce5ad]`;
  }

  if (status === "external-links") {
    return `${base} border-brand/40 bg-brand/15 text-brand`;
  }

  return `${base} border-foreground/20 bg-foreground/10 text-foreground/80`;
}
